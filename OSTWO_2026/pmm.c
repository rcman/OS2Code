...
static uint32_t highest_address = 0;  // top of highest usable RAM region
static uint32_t reserved_floor = 0x100000;  // frames below this are never freeable (BIOS + kernel)
...
void pmm_init(uint32_t mmap_addr, uint32_t mmap_length, uint32_t kernel_end) {
    ...
    // total_pages indexes the bitmap by absolute page frame number,
    // so it must cover up to the highest usable address (the memory
    // map has holes; using total_memory/PAGE_SIZE cut off top pages).
    total_pages = ADDR_TO_PFN(PAGE_ALIGN_DOWN(highest_address));
    ...
    reserved_floor = kernel_end_aligned;
...
void pmm_free_page(uint32_t phys_addr) {
    ...
    // The BIOS area and kernel image are permanently reserved. A free
    // in that range is always a caller bug - and if the bit were
    // cleared, the allocator would hand out kernel code as a "free"
    // page and the next write would destroy the running kernel.
    if (phys_addr < reserved_floor) {
        printf("[PMM] WARNING: refusing to free reserved frame 0x%x (caller 0x%x)\n",
               phys_addr, (uint32_t)__builtin_return_address(0));
        return;
    }
...
// Get the top of the highest usable RAM region
uint32_t pmm_get_highest_address(void) {
