...
// Screen bounds are taken from the active video mode. The old
// hardcoded 1024x768 bounds let the cursor coordinates run far past
// the 320x200 VGA screen, so the cursor could vanish off-screen and
// clicks mapped to nonexistent pixels in the default mode.
#include "vga_gfx.h"
#define SCREEN_WIDTH  (vga_get_mode_info()->width)
#define SCREEN_HEIGHT (vga_get_mode_info()->height)

// Mouse state
static int32_t mouse_x = 160;   // re-centered in mouse_init()
...
static void mouse_callback(registers_t* regs) {
    ...
    switch (mouse_cycle) {
        ...
        case 2:
            ...
            // Apply mouse sensitivity (3x multiplier for faster movement)
            dx *= 3;
            dy *= 3;
            ...
            // Generate release events on button release
            if (!left && mouse_buttons[0]) {
                input_event_t event;
                event.type = EVENT_TYPE_MOUSE_RELEASE;
                *(uint16_t*)&event.data[0] = (uint16_t)mouse_x;
                *(uint16_t*)&event.data[2] = (uint16_t)mouse_y;
                event.data[4] = 0;  // Left button
                push_event(event);
...
void mouse_init(void) {
    // Center cursor in the current video mode
