...
static process_t* find_next_process(void) {
    ...
    for (int i = 0; i < max * 2; i++) {
        ...
        // Only select READY processes (skip TERMINATED, UNUSED, BLOCKED)
        if (proc->state == PROCESS_STATE_READY && proc->pid != 0) {
            return proc;
        }
    }

    // No ready process found, return NULL
    // Don't switch to idle process - it's not a real user process
    return NULL;
}
...
void scheduler_schedule(void) {
    ...
    if (!next) {
        // No ready processes - disable scheduler and return to kernel
        printf("[Scheduler] No ready processes, stopping scheduler\n");
        scheduling_enabled = false;
        return;
    }

    // If same process, nothing to do (unless current is terminated)
    if (next == current && current->state != PROCESS_STATE_TERMINATED) {
        return;
    }
